...
app = Flask(__name__)
DB = os.path.join(os.path.dirname(__file__), 'database', 'fa.db')

# In-memory store for QR tokens: {token: {'session_id':..,'issued_at':..,'used': False}}
QR_TOKENS = {}
QR_LIFETIME = 25  # seconds a token is valid

def make_qr_token(session_id):
    """Generate a signed QR token for the given session."""
    nonce = secrets.token_hex(8)
    ts = int(time.time())
    raw = f"{session_id}:{ts}:{nonce}"
    sig = hashlib.sha256(raw.encode()).hexdigest()[:16]
    token = f"{raw}:{sig}"
    QR_TOKENS[token] = {'session_id': session_id, 'issued_at': ts, 'used': False}
    # Purge old tokens
    expired = [t for t, v in QR_TOKENS.items() if time.time() - v['issued_at'] > 300]
    for t in expired: del QR_TOKENS[t]
    return token

def verify_qr_token(token):
    """Returns (ok, session_id, error_msg)"""
    if token not in QR_TOKENS:
        return False, None, 'Invalid or expired QR code'
    rec = QR_TOKENS[token]
    if rec['used']:
        return False, None, 'QR token already used'
    age = time.time() - rec['issued_at']
    if age > QR_LIFETIME:
        return False, None, 'QR code expired — wait for teacher to refresh'
    # Verify signature
    parts = token.rsplit(':', 1)
    if len(parts) != 2: return False, None, 'Malformed token'
    raw, sig = parts
...
def get_qr_token(sid):
    ...
    return jsonify({'ok': True, 'token': token, 'lifetime': QR_LIFETIME})
...
if __name__ == '__main__':
    ...
    print(f'  Network: http://{ip}:5000  <- Share!\n{"="*52}\n')
