...
def stop_session(sid):
    ...
    present_list = [dict(r) for r in c.execute(
        'SELECT a.id AS aid,a.uid,a.name,a.roll,a.conf,u.img FROM attendance a LEFT JOIN users u ON a.uid=u.id WHERE a.sid=? AND a.status="present" ORDER BY a.name',
        (sid,)).fetchall()]
...
def session_summary(sid):
    ...
    present_list = [dict(r) for r in c.execute(
        'SELECT a.id AS aid,a.uid,a.name,a.roll,a.conf,u.img FROM attendance a LEFT JOIN users u ON a.uid=u.id WHERE a.sid=? AND a.status="present" ORDER BY a.name',(sid,)).fetchall()]
    absent_list = [dict(r) for r in c.execute(
