...
def start_session():
    d = request.json; c = db()
    tid = d.get('tid','')
    # Only deactivate THIS teacher's previous active sessions (allow multiple teachers simultaneously)
    c.execute('UPDATE sessions SET active=0, qr_open=0 WHERE tid=? AND active=1', (tid,))
    sid = ts()
    c.execute('INSERT INTO sessions VALUES(?,?,?,?,?,?,?,?,1,?,1)',
        (sid,d['did'],d['dname'],int(d['sem']),d['subj'],d['date'],
         tid,d['tname'],d.get('branch') or None))
...
def active_session():
    ...
    c = db()
    if tid:
        s = c.execute('SELECT * FROM sessions WHERE active=1 AND tid=? ORDER BY rowid DESC LIMIT 1',(tid,)).fetchone()
    else:
        s = c.execute('SELECT * FROM sessions WHERE active=1 ORDER BY rowid DESC LIMIT 1').fetchone()
    c.close()
...
def reset_db():
    """Admin only: wipe all attendance, sessions data (keep users/divisions/branches/cfg)"""
    key = request.json.get('key','')
    if key != 'RESET_CONFIRM':
        return jsonify({'ok': False, 'msg': 'Invalid confirmation key'})
    c = db()
    c.executescript('DELETE FROM attendance; DELETE FROM sessions;')
    c.commit(); c.close()
    return jsonify({'ok': True, 'msg': 'All attendance and session data has been cleared.'})
