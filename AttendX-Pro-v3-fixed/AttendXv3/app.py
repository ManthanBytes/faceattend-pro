...
def init_db():
    ...
    if USE_PG:
        for sql in [
            ...
            "CREATE TABLE IF NOT EXISTS users(id TEXT PRIMARY KEY,name TEXT,roll TEXT,email TEXT UNIQUE,pw TEXT,role TEXT DEFAULT 'student',div_id TEXT,sem INTEGER,dept_id TEXT,face TEXT,face_img TEXT,created_at TEXT)",
            ...
            "CREATE TABLE IF NOT EXISTS attendance(id TEXT PRIMARY KEY,session_id TEXT,student_id TEXT,name TEXT,roll TEXT,div_id TEXT,div_name TEXT,sem INTEGER,subject TEXT,date TEXT,day TEXT,status TEXT,method TEXT,conf INTEGER,marked_at TEXT,cancel_reason TEXT DEFAULT '')",
        ...
        c.executescript('''
            ...
            CREATE INDEX IF NOT EXISTS idx_tt_div   ON timetable(div_id,sem,day);
        ''')
        for sql in ['ALTER TABLE attendance ADD COLUMN cancel_reason TEXT DEFAULT ""','ALTER TABLE sessions ADD COLUMN qr_lifetime INTEGER DEFAULT 30','ALTER TABLE sessions ADD COLUMN att_mode TEXT DEFAULT "both"']:
            try: c.execute(sql)
            except: pass
        c.commit()
    if not c.fetchone('SELECT 1 FROM users LIMIT 1'):
        ...
        c.executemany('INSERT INTO depts VALUES(?,?,?)',[(d1,'Computer Science','CS'),(d2,'BCA','BCA')])
        c.executemany('INSERT INTO divisions VALUES(?,?,?,?)',[(v1,'CS Div-A',d1,8),(v2,'CS Div-B',d1,8),(v3,'BCA Div-A',d2,6),(v4,'BCA Div-B',d2,6)])
        ...
        c.executemany('INSERT INTO cfg VALUES(?,?)',[('college','My Engineering College'),('dept','Academics'),('min_att','75'),('face_conf','55'),('qr_lifetime','30'),('periods','6')])
        ...
        for di,day in enumerate(DAYS[:5]):
            for pi in range(6):
                c.execute('INSERT INTO timetable VALUES(?,?,?,?,?,?,?,?,?,?,?,?)',(new_id(),v1,3,d1,day,pi+1,times[pi][0],times[pi][1],subjs[(di+pi)%6],t1,'Prof. Sharma','Room 101'))
        c.commit()
    c.close()
...
def login():
    ...
    u=c.fetchone('SELECT * FROM users WHERE email=? AND pw=? AND role=?',(d['email'].lower().strip(),hp(d['password']),d['role']))
    c.close();return jsonify({'ok':bool(u),'user':u,'msg':'' if u else 'Wrong credentials'})
...
def register():
    ...
    uid=new_id()
    c.execute('INSERT INTO users VALUES(?,?,?,?,?,?,?,?,?,?,?,?)',(uid,name,roll,email,hp(pw),role,d.get('div_id'),int(d['sem']) if d.get('sem') else None,d.get('dept_id'),None,None,now_iso()))
    c.commit();u=c.fetchone('SELECT * FROM users WHERE id=?',(uid,));c.close();return jsonify({'ok':True,'user':u})
...
def get_users():
    ...
    q+=' ORDER BY name';r=c.fetchall(q,p);c.close();return jsonify(r)
...
def add_user():
    ...
    uid=new_id()
    c.execute('INSERT INTO users VALUES(?,?,?,?,?,?,?,?,?,?,?,?)',(uid,d['name'].strip(),roll,d['email'].lower(),hp(d.get('password','pass@123')),role,d.get('div_id'),int(d['sem']) if d.get('sem') else None,d.get('dept_id'),None,None,now_iso()))
    c.commit();c.close();return jsonify({'ok':True,'id':uid})

@app.route('/api/users/<uid>',methods=['GET'])
def get_user(uid):
    c=get_db();u=c.fetchone('SELECT * FROM users WHERE id=?',(uid,));c.close();return jsonify({'ok':bool(u),'user':u})
...
def end_session(sid):
    ...
    present_list=c.fetchall("SELECT a.id as att_id,a.student_id,a.name,a.roll,a.conf,a.method,u.face_img FROM attendance a LEFT JOIN users u ON u.id=a.student_id WHERE a.session_id=? AND a.status='present' ORDER BY a.name",(sid,))
...
def session_summary(sid):
    ...
    present_list=c.fetchall("SELECT a.id as att_id,a.student_id,a.name,a.roll,a.conf,a.method,u.face_img FROM attendance a LEFT JOIN users u ON u.id=a.student_id WHERE a.session_id=? AND a.status='present' ORDER BY a.name",(sid,))
    absent_list=c.fetchall("SELECT student_id,name,roll FROM attendance WHERE session_id=? AND status='absent' ORDER BY name",(sid,))
    cancelled_list=c.fetchall("SELECT a.id as att_id,a.student_id,a.name,a.roll,a.method,a.cancel_reason,u.face_img FROM attendance a LEFT JOIN users u ON u.id=a.student_id WHERE a.session_id=? AND a.status='cancelled' ORDER BY a.name",(sid,))
    c.close()
...
def verify_qr():
    ...
    if time.time()-float(rec['issued_at'])>600:
        c.execute('DELETE FROM qr_tokens WHERE token=?',(token,));c.commit();c.close();return jsonify({'ok':False,'msg':'Token expired — get new QR'})
    sid=rec['session_id']
    ...
    c.close();return jsonify({'ok':True,'verified':True,'mode':mode,'session':sess,'student':student,'msg':f'Token verified! Now scan your face for {sess["subject"]}'})
...
def mark_face():
    ...
    c=get_db();cfg_r={x['k']:x['v'] for x in c.fetchall('SELECT * FROM cfg')};thresh=float(cfg_r.get('face_conf',55))/100
    student=c.fetchone('SELECT * FROM users WHERE id=?',(student_id,))
    ...
    c.execute('INSERT INTO attendance VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)',(new_id(),session_id,student_id,student['name'],student['roll'],sess['div_id'],sess['div_name'],sess['sem'],sess['subject'],sess['date'],sess.get('day',''),'present','face+qr',score,now_iso(),''))
    c.commit();c.close();return jsonify({'ok':True,'marked':True,'name':student['name'],'conf':score,'img':student.get('face_img'),'msg':f'Present marked — {sess["subject"]} ({score}% match)'})
