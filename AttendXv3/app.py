...
def init_db():
    ...
    if USE_PG:
        ...
        c.executescript('''
            ...
            CREATE INDEX IF NOT EXISTS idx_tt_div   ON timetable(div_id,sem,day);
        ''')
        for sql in ['ALTER TABLE attendance ADD COLUMN cancel_reason TEXT DEFAULT ""','ALTER TABLE sessions ADD COLUMN qr_lifetime INTEGER DEFAULT 30','ALTER TABLE sessions ADD COLUMN att_mode TEXT DEFAULT "both"','ALTER TABLE users ADD COLUMN status TEXT DEFAULT "active"']:
            try: c.execute(sql)
            except: pass
        c.commit()
