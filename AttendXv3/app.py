...
def get_db():
    ...
    c.execute('PRAGMA cache_size=50000')   # ~200MB cache
    c.execute('PRAGMA temp_store=MEMORY')
    c.execute('PRAGMA mmap_size=536870912') # 512MB mmap
    c.execute('PRAGMA busy_timeout=10000')  # 10s wait on locked DB
    c.execute('PRAGMA foreign_keys=OFF')    # speed
    return c
...
def init_db():
    ...
    c.executescript('''
        ...
        -- Timetable: one row per period per day per division
        ...
        -- Add qr_lifetime column if missing (migration for existing DBs)
        CREATE TABLE IF NOT EXISTS _dummy_migration(x);
    ''')
    # Run migrations separately
    try: c2=get_db(); c2.execute('ALTER TABLE sessions ADD COLUMN qr_lifetime INTEGER DEFAULT 30'); c2.commit(); c2.close()
    except: pass
    ...
    try: c.execute('ALTER TABLE attendance ADD COLUMN cancel_reason TEXT DEFAULT ""')
    except: pass
    c.commit()
