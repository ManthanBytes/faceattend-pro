    ...
    if face_ticket:
        ...
        if len(parts)>=3:
            ...
            if time.time()>exp_ts:
                ...
                c.close();return jsonify({'ok':False,'msg':'Face scan time expired (3 min limit) — scan QR again','reason':'ticket_expired'})
        ...
        c.execute('DELETE FROM qr_tokens WHERE token=?',(face_key,));c.commit()
    # else: no ticket — allow (backwards compat for QR-only mode etc.)

    cfg_r={x['k']:x['v'] for x in c.fetchall('SELECT * FROM cfg')};thresh=float(cfg_r.get('face_conf',55))/100
