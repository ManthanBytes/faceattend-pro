...
def login():
    ...
    u=c.fetchone('SELECT id,name,roll,email,role,div_id,sem,dept_id,face_img,created_at,CASE WHEN face IS NOT NULL THEN 1 ELSE 0 END as has_face FROM users WHERE email=? AND pw=? AND role=?',(d['email'].lower().strip(),hp(d['password']),d['role']))
