...
# Priority: 1) Env var  2) /data (Render persistent disk)  3) local database/  4) /tmp
def get_db_path():
    ...
    if os.path.isdir('/data'):
        return '/data/fa.db'
    # Local development
    local = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'database', 'fa.db')
    ...
    try:
        ...
        pass
    # Last resort — /tmp (will reset on restart but won't crash)
    return '/tmp/fa.db'

DB = get_db_path()
print(f'[DB] Using database: {DB}')
...
def init():
    ...
    if not c.execute('SELECT 1 FROM users LIMIT 1').fetchone():
        ...
        c.executemany('INSERT OR IGNORE INTO users VALUES(?,?,?,?,?,?,?,?,?,?,?)', [
            ('u1','Admin HOD','ADM001','admin@college.edu',hp('admin@123'),'admin',None,None,None,None,None),
            ('u2','Prof. Sharma','TCH001','teacher@college.edu',hp('admin@123'),'teacher','d1',3,None,None,'b1'),
...
def stop_session(sid):
    ...
    present_list = [dict(r) for r in c.execute(
        'SELECT a.id AS aid,a.uid,a.name,a.roll,a.conf,a.at,u.img FROM attendance a LEFT JOIN users u ON a.uid=u.id WHERE a.sid=? AND a.status="present" ORDER BY a.name',
        (sid,)).fetchall()]
...
def session_summary(sid):
    ...
    present_list = [dict(r) for r in c.execute(
        'SELECT a.id AS aid,a.uid,a.name,a.roll,a.conf,a.at,u.img FROM attendance a LEFT JOIN users u ON a.uid=u.id WHERE a.sid=? AND a.status="present" ORDER BY a.name',(sid,)).fetchall()]
    absent_list = [dict(r) for r in c.execute(
...
def delete_attendance(aid):
    c = db()
    rec = c.execute('SELECT * FROM attendance WHERE id=?',(aid,)).fetchone()
    if not rec: c.close(); return jsonify({'ok':False,'msg':'Not found'})
    # Mark as absent instead of deleting so record is preserved
    c.execute("UPDATE attendance SET status='absent',conf=NULL,method='cancelled' WHERE id=?",(aid,))
    c.commit(); c.close()
    return jsonify({'ok':True})
...
def reset_admin():
    """Emergency: reset admin & teacher passwords to admin@123"""
    c = db()
    c.execute("UPDATE users SET ph=? WHERE role='admin'", (hp('admin@123'),))
    c.execute("UPDATE users SET ph=? WHERE role='teacher'", (hp('admin@123'),))
    c.commit(); c.close()
    return jsonify({'ok': True, 'msg': 'Admin & Teacher passwords reset to admin@123'})
