...
def db_route(f):
    ...
    def wrapper(*args,**kwargs):
        try:
            ...
            return jsonify({'ok':False,'msg':f'Server error: {str(e)}'}),500
    return wrapper
...
def get_user(uid):
    c=get_db();u=c.fetchone('SELECT id,name,roll,email,role,div_id,sem,dept_id,face_img,created_at,status,CASE WHEN face IS NOT NULL THEN 1 ELSE 0 END as has_face FROM users WHERE id=?',(uid,));c.close()
...
def update_user(uid):
    d=request.json;c=get_db()
    fields=[];vals=[]
    for k in ['name','roll','email','div_id','sem','dept_id','status']:
        if k in d: fields.append(f'{k}=?');vals.append(d[k])
    if 'password' in d and d['password']: fields.append('pw=?');vals.append(hp(d['password']))
    if fields:
        vals.append(uid);c.execute(f'UPDATE users SET {",".join(fields)} WHERE id=?',vals);c.commit()
    c.close();return jsonify({'ok':True})

@app.route('/api/users/<uid>',methods=['DELETE'])
def del_user(uid):
    c=get_db();c.execute('DELETE FROM users WHERE id=?',(uid,));c.commit();c.close();return jsonify({'ok':True})

@app.route('/api/users/pending')
def pending_users():
    c=get_db();r=c.fetchall("SELECT id,name,roll,email,role,div_id,sem,dept_id,created_at FROM users WHERE status='pending' ORDER BY created_at DESC");c.close();return jsonify(r)

@app.route('/api/teachers/pending')
def pending_teachers():
    c=get_db();r=c.fetchall("SELECT id,name,roll,email,role,div_id,sem,dept_id,created_at FROM users WHERE status='pending' AND role='teacher' ORDER BY created_at DESC");c.close();return jsonify(r)

@app.route('/api/users/<uid>/approve',methods=['POST'])
def approve_user(uid):
    c=get_db();c.execute("UPDATE users SET status='active' WHERE id=?",(uid,));c.commit();c.close();return jsonify({'ok':True})

@app.route('/api/users/<uid>/reject',methods=['POST'])
def reject_user(uid):
    c=get_db();c.execute("UPDATE users SET status='rejected' WHERE id=?",(uid,));c.commit();c.close();return jsonify({'ok':True})

@app.route('/api/enroll-face',methods=['POST'])
def enroll_face():
    d=request.json;uid=d.get('user_id');face=d.get('face');img=d.get('img')
    if not uid or not face: return jsonify({'ok':False,'msg':'Missing data'})
    c=get_db();c.execute('UPDATE users SET face=?,face_img=? WHERE id=?',(json.dumps(face),img,uid));c.commit();c.close();return jsonify({'ok':True})

@app.route('/api/sessions',methods=['POST'])
def start_session():
    d=request.json;c=get_db()
    ex=c.fetchone('SELECT id FROM sessions WHERE teacher_id=? AND active=1',(d['teacher_id'],))
    if ex: c.execute('UPDATE sessions SET active=0,ended_at=? WHERE id=?',(now_iso(),ex['id']));c.commit()
    sid=new_id()
    c.execute('INSERT INTO sessions(id,teacher_id,teacher_name,div_id,div_name,dept_id,sem,subject,room,date,day,started_at,active,qr_open,att_mode,qr_lifetime) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,1,0,?,?)',(sid,d['teacher_id'],d['teacher_name'],d['div_id'],d['div_name'],d.get('dept_id',''),int(d['sem']),d['subject'],d.get('room',''),today(),today_day(),now_iso(),d.get('att_mode','both'),int(d.get('qr_lifetime',30))))
    c.commit();sess=c.fetchone('SELECT * FROM sessions WHERE id=?',(sid,));c.close();return jsonify({'ok':True,'id':sid,'session':sess})

@app.route('/api/sessions/<sid>',methods=['PUT'])
def update_session(sid):
    d=request.json;c=get_db()
    fields=[];vals=[]
    for k in ['qr_open','att_mode','qr_lifetime']:
        if k in d: fields.append(f'{k}=?');vals.append(d[k])
    if 'active' in d:
        fields.append('active=?');vals.append(int(d['active']))
    if 'ended_at' in d: fields.append('ended_at=?');vals.append(d['ended_at'])
    # Auto set ended_at when session is being ended
    if d.get('active') in (0,'0',False) and 'ended_at' not in d:
        fields.append('ended_at=?');vals.append(now_iso())
    if not fields:
        sess=c.fetchone('SELECT * FROM sessions WHERE id=?',(sid,));c.close();return jsonify({'ok':True,'session':sess})
    vals.append(sid)
    c.execute(f'UPDATE sessions SET {",".join(fields)} WHERE id=?',vals);c.commit()
    sess=c.fetchone('SELECT * FROM sessions WHERE id=?',(sid,))
    # When ending session, mark absent students who didn't attend
    if d.get('active') in (0,'0',False) and sess:
        students=c.fetchall("SELECT id,name,roll,div_id,sem FROM users WHERE role='student' AND div_id=? AND sem=?",(sess['div_id'],sess['sem']))
        for s in students:
            already=c.fetchone('SELECT 1 FROM attendance WHERE session_id=? AND student_id=?',(sid,s['id']))
            if not already:
                c.execute('INSERT INTO attendance(id,session_id,student_id,name,roll,div_id,div_name,sem,subject,date,day,status,method,conf,marked_at,cancel_reason) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)',
                    (new_id(),sid,s['id'],s['name'],s['roll'],sess['div_id'],sess['div_name'],sess['sem'],sess['subject'],sess['date'],sess.get('day',''),'absent','auto',None,now_iso(),''))
        c.commit()
    c.close();return jsonify({'ok':True,'session':sess})

@app.route('/api/sessions/<sid>',methods=['GET'])
def get_session(sid):
    c=get_db();s=c.fetchone('SELECT * FROM sessions WHERE id=?',(sid,));c.close();return jsonify(s or {})

@app.route('/api/sessions')
def get_sessions():
    c=get_db();q='SELECT * FROM sessions WHERE 1=1';p=[]
    for k in ['teacher_id','div_id','sem','active','date']:
        if request.args.get(k): q+=f' AND {k}=?';p.append(request.args[k])
    q+=' ORDER BY started_at DESC LIMIT 200';r=c.fetchall(q,p);c.close();return jsonify(r)

@app.route('/api/sessions/<sid>/gen-qr',methods=['POST'])
...
def gen_qr(sid):
    ...
    lifetime=int(sess.get('qr_lifetime') or 30)
    if USE_PG:
...
def session_summary(sid):
    ...
    total_r=c.fetchone("SELECT COUNT(*) as n FROM users WHERE role='student' AND div_id=? AND sem=?",(sess['div_id'],sess['sem']))
...
def verify_qr():
    ...
    if mode=='qr':
        c.execute('INSERT INTO attendance(id,session_id,student_id,name,roll,div_id,div_name,sem,subject,date,day,status,method,conf,marked_at,cancel_reason) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)',(new_id(),sid,student_id,student['name'],student['roll'],sess['div_id'],sess['div_name'],sess['sem'],sess['subject'],sess['date'],sess.get('day',''),'present','qr',None,now_iso(),''))
...
def mark_face():
    ...
    if face_ticket:
        ...
        if len(parts)>=3:
            ...
            if time.time()>exp_ts:
                ...
                c.close();return jsonify({'ok':False,'msg':'Face scan time expired (3 min limit) — please scan QR again','reason':'ticket_expired'})
    ...
    if sim<thresh:
        if score>=req-5:
            hint=f'Face too close to boundary ({score}% match, need >{req}%). Make sure your full face is on screen.'
        elif score>40:
            hint='Hold phone steady, improve lighting. Make sure your full face is visible.'
        else:
            hint='Try re-enrolling your face in good lighting.'
        c.close();return jsonify({'ok':False,'reason':'low_match','msg':f'Face match too low ({score}%, need ≥{req}%) — ensure full face is on screen','score':score,'required':req,'hint':hint,'img':student.get('face_img')})
    ...
    att_img=thumb or student.get('face_img')
...
def end_session(sid):
    ...
    sess=c.fetchone('SELECT * FROM sessions WHERE id=?',(sid,))
    if not sess: c.close(); return jsonify({'ok':False,'msg':'Session not found'})
    c.execute('UPDATE sessions SET active=0,ended_at=?,qr_open=0 WHERE id=?',(now_iso(),sid))
    c.commit()
    # Mark absent for students who didn't attend
    students=c.fetchall("SELECT id,name,roll FROM users WHERE role='student' AND div_id=? AND sem=?",(sess['div_id'],sess['sem']))
    for s in students:
        already=c.fetchone('SELECT 1 FROM attendance WHERE session_id=? AND student_id=?',(sid,s['id']))
        if not already:
            c.execute('INSERT INTO attendance(id,session_id,student_id,name,roll,div_id,div_name,sem,subject,date,day,status,method,conf,marked_at,cancel_reason) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)',
    ...
    cancelled_list=c.fetchall("SELECT a.id as att_id,a.student_id,a.name,a.roll,a.method,a.cancel_reason,COALESCE(a.att_img,u.face_img) as face_img FROM attendance a LEFT JOIN users u ON u.id=a.student_id WHERE a.session_id=? AND a.status='cancelled' ORDER BY a.name",(sid,))
    c.close()
    summary={'session':sess,'total':total,'present':len(present_list),'absent':len(absent_list),'cancelled':len(cancelled_list),'present_list':present_list,'absent_list':absent_list,'cancelled_list':cancelled_list}
    return jsonify({'ok':True,'session':sess,'summary':summary})
...
def open_qr(sid):
    ...
    sess=c.fetchone('SELECT * FROM sessions WHERE id=? AND active=1',(sid,))
    if not sess: c.close(); return jsonify({'ok':False,'msg':'Session not active'})
    c.execute('UPDATE sessions SET qr_open=1 WHERE id=?',(sid,))
    c.commit()
...
def all_teachers():
    c=get_db()
    teachers=c.fetchall("SELECT id,name,roll,email,div_id,sem,dept_id,created_at,status,CASE WHEN face IS NOT NULL THEN 1 ELSE 0 END as has_face FROM users WHERE role='teacher' ORDER BY name")
    for t in teachers:
        sc=c.fetchone('SELECT COUNT(*) as n FROM sessions WHERE teacher_id=?',(t['id'],))
        t['session_count']=sc['n'] if sc else 0
    c.close()
...
def approve_teacher(uid):
    c=get_db();c.execute("UPDATE users SET status='active' WHERE id=? AND role='teacher'",(uid,));c.commit();c.close();return jsonify({'ok':True})

@app.route('/api/teachers/<uid>/reject', methods=['POST'])
def reject_teacher(uid):
    c=get_db();c.execute("UPDATE users SET status='rejected' WHERE id=? AND role='teacher'",(uid,));c.commit();c.close();return jsonify({'ok':True})

@app.route('/api/teachers/<uid>/revoke', methods=['POST'])
def revoke_teacher(uid):
    c=get_db();c.execute("UPDATE users SET status='pending' WHERE id=? AND role='teacher'",(uid,));c.commit();c.close();return jsonify({'ok':True})

@app.route('/api/timetable/<div_id>/<int:sem>', methods=['GET'])
def get_timetable_by_div(div_id, sem):
    c=get_db();rows=c.fetchall('SELECT * FROM timetable WHERE div_id=? AND sem=? ORDER BY day,period',(div_id,sem));c.close();return jsonify(rows)
...
def _keep_alive():
    ...
    def ping():
        while True:
            try:
                url=os.environ.get('RENDER_EXTERNAL_URL') or os.environ.get('VERCEL_URL','')
                if url:
