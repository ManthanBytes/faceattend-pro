...
def get_user(uid):
    c=get_db();u=c.fetchone('SELECT id,name,roll,email,role,div_id,sem,dept_id,face_img,created_at,status,CASE WHEN face IS NOT NULL THEN 1 ELSE 0 END as has_face FROM users WHERE id=?',(uid,));c.close();return jsonify(u or {})
...
def start_session():
    ...
    c.commit();sess=c.fetchone('SELECT * FROM sessions WHERE id=?',(sid,));c.close();return jsonify({'ok':True,'session':sess})
...
def all_teachers():
    c=get_db()
    r=c.fetchall("SELECT id,name,roll,email,div_id,sem,dept_id,created_at,CASE WHEN face IS NOT NULL THEN 1 ELSE 0 END as has_face FROM users WHERE role='teacher' AND status='active' ORDER BY name")
    c.close()
    return jsonify(r)
