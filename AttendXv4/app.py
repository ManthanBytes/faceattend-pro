...
def update_session(sid):
    d=request.json;c=get_db()
    fields=[];vals=[]
    for k in ['active','qr_open','att_mode','qr_lifetime']:
        if k in d: fields.append(f'{k}=?');vals.append(d[k])
    if 'ended_at' in d: fields.append('ended_at=?');vals.append(d['ended_at'])
    # Auto set ended_at when session ended
    if d.get('active')==0 and 'ended_at' not in d:
        fields.append('ended_at=?');vals.append(now_iso())
    if fields: vals.append(sid);c.execute(f'UPDATE sessions SET {",".join(fields)} WHERE id=?',vals);c.commit()
